    ...
    (actor (thread (lambda ()
              (let loop ((behavior initial-behavior))
                (let ((msg (thread-receive)))
                  (loop (apply behavior (car msg) (cdr msg))))))))))
...
(define-syntax send/a
  (syntax-rules ()
    ((send/a receiver tag arg ...)
     (actor-send receiver (cons (quote tag) (list arg ...))))))
...
(struct message/c (expected-tag ensure-sends ensure-behaviors))

(define-syntax behavior/c 
  (syntax-rules ()
     ((behavior/c (contructor-contract ...) 
                 ((tag (param-contract ...)) ...))
     'todo)))

(define (create/c contract behavior . args) 'todo)

;; A send on a contract entity, is used internally by `send` when it can dispatch to a contracted send.
...
(define myplayer (create player 1))
