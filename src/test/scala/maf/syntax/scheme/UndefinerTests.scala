...
class UndefinerTests extends AnyPropSpec:
  ...
  def comparePrograms(program1: SchemeExp, program2: SchemeExp): Boolean =
    (program1, program2) match
      case (
            SchemeLambda(name, args, body, ann, _),
            SchemeLambda(name1, args1, body1, ann1, _)
          ) if name == name1 && args.zip(args1).forall { case (a, b) =>
            a.name == b.name
          } && ann == ann1 =>
        body.zip(body1).forall { case (a, b) =>
          a == b
        } // TODO: args also have an identity that should be ignored

      case (
            SchemeVarArgLambda(name, args, vararg, body, ann, _),
            SchemeVarArgLambda(name1, args1, vararg1, body1, ann1, _)
          ) if name == name1 && args.zip(args1).forall { case (a, b) =>
            a.name == b.name
          } && ann == ann1 && vararg == vararg1 =>
        body.zip(body1).forall { case (a, b) => a == b }

      case (SchemeFuncall(f, args, _), SchemeFuncall(f1, args1, _)) =>
        comparePrograms(f, f1) && args.zip(args1).forall { case (a, b) =>
          comparePrograms(a, b)
        }
      case (SchemeIf(cond, cons, alt, _), SchemeIf(cond1, cons1, alt1, _)) =>
        comparePrograms(cond, cond1) && comparePrograms(
          cons,
          cons1
        ) && comparePrograms(alt, alt1)

      case (SchemeLet(bindings, body, _), SchemeLet(bindings1, body1, _)) =>
        bindings.map(_._1.name).zip(bindings1.map(_._1.name)).forall {
          case (a, b) => a == b
        } && body.zip(body1).forall { case (body, body1) =>
          comparePrograms(body, body1)
        }

      case (
            SchemeLetStar(bindings, body, _),
            SchemeLetStar(bindings1, body1, _)
          ) =>
        bindings.map(_._1.name).zip(bindings1.map(_._1.name)).forall {
          case (a, b) => a == b
        } && body.zip(body1).forall { case (body, body1) =>
      ...
      case (
            SchemeLetrec(bindings, body, _),
            SchemeLetrec(bindings1, body1, _)
          ) =>
        bindings.map(_._1.name).zip(bindings1.map(_._1.name)).forall {
          case (a, b) => a == b
        } && body.zip(body1).forall { case (body, body1) =>
      ...
      case (
            SchemeSetLex(variable, _, value, _),
            SchemeSetLex(variable1, _, value1, _)
          ) =>
        variable.name == variable1.name && comparePrograms(value, value1)
      ...
      case (
            SchemeDefineVariable(name, value, _),
            SchemeDefineVariable(name1, value1, _)
          ) =>
  ...
  def test(name: String, original: String, translated: String): Unit =
    property(name) {
      ...
      val undefinedOriginal = SchemeBegin(
        SchemeMonadicUndefiner.undefineExps(parsedOriginal, false),
        Identity.none
      )
