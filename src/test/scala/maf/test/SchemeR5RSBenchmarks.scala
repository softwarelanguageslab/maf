...
object SchemeR5RSBenchmarks:
  ...
  val bench: List[(String, Any)] = List(
    ...
    (
      "(equal? (append '(a b) (cons 'c 'd)) (cons 'a (cons 'b (cons 'c 'd))))",
      true
    ),
    ...
    // (make-string k)
    (
      "(string-length (make-string 5))",
      5
    ), // The content of the string is unspecified
    // (make-string k char)
    ...
    (
      "(begin (define x \"foo\") (string-fill! x #\\a) (equal? x \"aaa\"))",
      true
    ),
    // §6.3.6 Vectors
    // (vector? obj)
    ...
    (
      "(equal? (let ((v (make-vector 5))) (for-each (lambda (i) (vector-set! v i (* i i))) '(0 1 2 3 4)) v) (vector 0 1 4 9 16))",
      true
    )
