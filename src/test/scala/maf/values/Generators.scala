...
//    val list = set.toList
//    for n <- Gen.choose(0, set.size)
//    yield scala.util.Random.shuffle(list).take(n).toSet
//
//class ConcreteGenerator[T](g: Gen[T])(implicit lat: Lattice[Concrete.L[T]])
//    extends LatticeGenerator[Concrete.L[T]]:
...
//    case Concrete.Values(content) =>
//      isetgen.genSubset(content.toSet[T]).map(x => Concrete.Values(x))
...
//  }
//
//object ConcreteStringGenerator
//    extends ConcreteGenerator[String](Generators.str)(Concrete.L.stringConcrete)
//
//object ConcreteIntGenerator extends ConcreteGenerator[BigInt](Generators.int)
//
//object ConcreteRealGenerator
//    extends ConcreteGenerator[Double](Generators.double)
//object ConcreteCharGenerator extends ConcreteGenerator[Char](Generators.char)
//object ConcreteSymbolGenerator
//    extends ConcreteGenerator[String](Generators.sym)(Concrete.L.symConcrete)
...
abstract class ConstantPropagationGenerator[X](gen: Gen[X])(implicit
    lat: Lattice[ConstantPropagation.L[X]]
) extends LatticeGenerator[ConstantPropagation.L[X]]:
  def constgen: Gen[ConstantPropagation.L[X]] = for x <- gen
  yield ConstantPropagation.Constant(x)
...
object ConstantPropagationStringGenerator
    extends ConstantPropagationGenerator[String](Generators.str)(
      ConstantPropagation.L.stringCP
    )

object ConstantPropagationIntGenerator
    extends ConstantPropagationGenerator[BigInt](Generators.int)

object ConstantPropagationRealGenerator
    extends ConstantPropagationGenerator[Double](Generators.double)
