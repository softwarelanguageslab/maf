...
class SparseProduct[P: Product] private (
  ...
  def get[K, V](key: K)(using
      KeyValueIn[K, V, P]
  ): Option[V] = map.get(SparseProductKey(key))
  ...
  def extend[K, V, P](key: K, vlu: V)(using
      Product[(K ~> V) :*: P]
  ): Self[(K ~> V) :*: P] =
    SparseProduct(map.put(SparseProductKey(key), vlu))
  ...
  /** Extend the product type contained within this map, but do not extend the
    * contents itself
    */
  def extend[K, V, P](using
      NoDuplicates[(K ~> V) :*: P]
  ): Self[(K ~> V) :*: P] = this.asInstanceOf[Self[(K ~> V) :*: P]]

  /** Checks whether the key is currently in the product, fails at compile-time
    * if the type of the product does not contain the key
    */
...
trait Rest[P, M]:
  def rest(p: P): M
object Rest:
  given [K, V, P]: Rest[SparseProduct[(K ~> V) :*: P], SparseProduct[P]] with {
...
private class TaggedTuple[P: Product] private (
  ...
  def get[K, V](key: K)(using
      KeyValueIn[K, V, P]
  ): V = map.get(SparseProductKey(key)).get
