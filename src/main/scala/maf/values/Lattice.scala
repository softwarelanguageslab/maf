...
trait Lattice[L] extends PartialOrdering[L] with Show[L] with Serializable:
    ...
    /** Elements of the lattice can be joined together
      ...
      *   parameter is evaluated upon _every_ use. This has two consequences:
      *   (1) the computation of `y` may be performed multiple times, and (2)
      *   the computation of `y` may not be performed at all, in which case
    ...
    /** Subsumption between two elements can be checked, y ⊑ x */
...
object Lattice:
    def apply[L: Lattice]: Lattice[L] = summon
