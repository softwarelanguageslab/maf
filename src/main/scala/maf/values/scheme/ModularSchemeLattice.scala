...
type PaiT = PaiT.type
case object VecPtrT extends Key[VecPtrT]
type VecPtrT = VecPtrT.type
case object PairPtrT extends Key[PairPtrT]
type PairPtrT = PairPtrT.type
...
type ModularSchemeValue[I, R, B, S, C, Sym, V, P] =
  (IntT ~> I) :*:
    ...
    (VecPtrT ~> Set[VectorAddress[V]]) :*:
    (PairPtrT ~> Set[PairPtrT[P]])
...
trait ModularSchemeLattice[
  ...
  private def insertA[K, V](
      k: K
    )(v: V
    )(using KeyValueIn[K, V, Val#Content]
    ): Val =
  ...
  private def setExtractor[K, V <: Set[A], A](
      k: K
    )(using KeyValueIn[K, V, Val#Content]
    ): Extractor[Val, A] =
    new Extractor:
  ...
  override def length[M[_]: MonadError: MonadJoin](s: Val): M[Val] =
    MonadJoin[M].mfoldMap(split(s)) {
      case StringT(s) =>
        StringLattice[S, I, C, Sym].length(s) map insertA(IntT)
      case v =>
        raiseError(
          TypeError("string-length: argument is not a string", v)
        )
    }
  ...
  override def append[M[_]: MonadError: MonadJoin](s1: Val, s2: Val): M[Val] =
    MonadJoin[M].mfoldMap(split(s1).cartesian(split(s2))) {
      ...
      case (v1, v2) =>
        raiseError(
          TypeError("append: arguments must be strings", (v1, v2))
        )
    }
  ...
  override def substring[M[_]: MonadError: MonadJoin](
    ...
    MonadJoin[M].mfoldMap(split(s).cartesian(split(from)).cartesian(split(to))) {
      case ((StringT(s1), IntT(i1)), IntT(i2)) =>
        StringLattice[S, I, C, Sym].substring(s1, i1, i2) map insertA(
          StringT
        )
      case v =>
        raiseError(
  ...
  override def set[M[_]: MonadError: MonadJoin](
    ...
    MonadJoin[M].mfoldMap(
      split(s).cartesian(split(i)).cartesian(split(c))
    ) {
  ...
  private def numOperation2[M[_]: MonadError: MonadJoin](
    ...
    MonadJoin[M].mfoldMap(split(n1).cartesian(split(n2))) {
      ...
      case (IntT(n1), RealT(n2)) =>
        ((IntLattice[I].toReal(n1), n2.pure) flatMapN ((r1: R, r2: R) => realOp(r1, r2))) map insertA[RealT, R](
          RealT
        )
      case (RealT(n1), IntT(n2)) =>
        ((n1.pure, IntLattice[I].toReal(n2)) flatMapN ((r1: R, r2: R) => realOp(r1, r2))) map insertA(RealT)
  ...
  private def numOperation2B[M[_]: MonadError: MonadJoin, B: BoolLattice](
    ...
    MonadJoin[M].mfoldMap(split(n1).cartesian(split(n2))) {
      ...
      case (IntT(n1), RealT(n2)) =>
        ((IntLattice[I].toReal(n1), n2.pure) flatMapN ((r1: R, r2: R) => realOp(r1, r2)))
      case (RealT(n1), IntT(n2)) =>
        ((n1.pure, IntLattice[I].toReal(n2)) flatMapN ((r1: R, r2: R) => realOp(r1, r2)))
      case (RealT(n1), RealT(n2)) =>
        realOp(n1, n2)
      case v =>
        raiseError(TypeError(s"$name: expected int or real", v))
  ...
  private def numOperation1[M[_]: MonadError: MonadJoin](
    ...
    MonadJoin[M].mfoldMap(split(n)) {
      case IntT(n) =>
        (IntLattice[I].toReal(n) >>= realOp) map insertA(RealT)
...
type CPSchemeValue = ModularSchemeValue[
  ...
  AbstractVector[CPSchemeValue, CP[BigInt]]
]
