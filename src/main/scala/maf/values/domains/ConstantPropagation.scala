...
import cats.{syntax => _, MonadError => _, *}
import cats.data.*
...
/** An implementation of a (rather standard) constant propagation domain. It implements all the lattice typeclasses from
  * `maf.values.typeclasses`.
  *
  * The domain consists of three values:
  *   - Bottom: the smallest element of the lattice, it is subsumed by any other value
  *   - Constant(x): denotes a constant value `x`
  *   - Top: denotes a non-constant value
  *
  * All operations are expressed using this lattice for simplicity. This means that a `string-ref` operations is only
  * supported for CP string values, and CP int values, and no combination of other values.
  */
object ConstantPropagation:
  ...
  /** All elements of the lattice are of type L
    *
    * @note
    *   The type parameter is covariant such that if Top <: L[Nothing] the following is true: L[Nothing] <: L[A] for any
    *   A
    */
  sealed trait L[+A]
  ...
  /** Map a function `f` over the value contained within `v`.
    *
    * @note
    *   If any of the elements is `Bottom` the result of the computation is also `Bottom`
    */
  ...
  /** Provides base lattice operations for any constantly propagated value
    *
    * @param typeName
    *   the name of the type contained within the value, used for printing purposes.
    */
  ...
  /** A thin wrapper around strings and symbols in order to differentiate them on a type level
    */
  ...
  object L:
    ...
    implicit val stringCP: StringLattice[S, I, C, Sym] =
      new BaseInstance[String]("Str") with StringLattice[S, I, C, Sym] {
        ...
        def append[M[_]: MonadError[Error]: MonadJoin](
            s1: S,
            s2: S
          ): M[S] =
          ((s1, s2) match
          ...
          case (Constant(s), from, to)                      =>
            ...
            (0.to(s.size)
              .collect({
                case from2
                    if BoolLattice[B]
                      .isTrue(IntLattice[I].eql[B](from, Galois.inject[BigInt, I](from2))) =>
                  (from2
                    .to(s.size)
                    .collect({
                      case to2
        ...
        def stringLt[M[_]: MonadError[Error]: MonadJoin, B2: BoolLattice: GaloisFrom[Boolean]](s1: S, s2: S): M[B2] =
          ((s1, s2) match
          ...
          s match
            ...
            case Constant(str) =>
              Errors.liftFromOption[M](
                NumOps
                  .bigIntFromString(str)
                  .map(Galois.inject[BigInt, I]),
                NotANumberString
              )
            case Top =>
              IntLattice[I].top.pure || ApplicativeError[M, Error]
                .raiseError(
                  NotANumberString
                )
      }
    ...
    implicit val intCP: IntLattice[I] = new BaseInstance[BigInt]("Int") with IntLattice[I] {
      ...
      def toReal[
          ...
          R2: RealLattice: GaloisFrom[Double]
        ](n: I
        ): M[R2] = (n match
      ...
      def div[M[_], R2: GaloisFrom[Double]](
        ...
        case (_, Top) =>
          RealLattice[R2].top.pure || raiseError(
      ...
      def expt[M[_]: MonadError[Error]: MonadJoin](n1: I, n2: I): M[I] =
        ((n1, n2) mapN ((n1, n2) => Math.pow(n1.toDouble, n2.toDouble).toInt): I)
          .pure[M]
      ...
      def toString[
          ...
          S: StringLattice_[I2, C, Sym]: GaloisFrom[String]
        ](n: I
        ): S =
        n match
      ...
      def toChar[
          C: CharLattice_[I, Sym, S]: GaloisFrom[
            Char
          ],
          S: StringLattice_[
            I,
    ...
    implicit val realCP: RealLattice[R] = new BaseInstance[Double]("Real") with RealLattice[R] {
      ...
      def asin[M[_]: MonadError[Error]: MonadJoin](n: R): M[R] =
        MonadJoin[M].cond(n map (i => -1 <= i && i <= 1)) {
          ...
          raiseError("asin input must be in range of -1 and 1")
        }
      ...
      def acos[M[_]: MonadError[Error]: MonadJoin](n: R): M[R] =
        MonadJoin[M].cond(n map (i => -1 <= i && i <= 1)) {
          ...
          raiseError("acos input must be in range of -1 and 1")
        }
      ...
      def toString[
          ...
          S: StringLattice_[I2, C2, Sym2]: GaloisFrom[String]
        ](c: R
        ): S =
        c match
    ...
    implicit val charCP: CharLattice[C, I, Sym, S] =
      new BaseInstance[Char]("Char") with CharLattice[C, I, Sym, S] {
        ...
        def toString[
            ...
            S: StringLattice_[I2, C2, Sym]: GaloisFrom[String]
          ](c: C
          ): S =
          c match
        ...
        def toInt[
            ...
            I2: IntLattice: GaloisFrom[BigInt]
          ](c: C
        ...
        def isLower[
            ...
            B2: BoolLattice: GaloisFrom[Boolean]
          ](c: C
