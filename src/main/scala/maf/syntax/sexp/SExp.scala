...
object Value:
  ...
  case class Real(value: Double) extends Value:
    override def toString = f"$value%e"
      .replace(",", ".")
      .nn // Might not preserve full precision, but will be in a Scheme-compatible format
...
/** Abstract grammar elements for S-expressions include some positional
  ...
  * machine.
  */
sealed trait SExp extends Expression[SExp]:
  val idn: Identity
  def fv: Set[String] = Set()

/** An s-expression is made of pairs, e.g., (foo bar) is represented as the pair
  * with identifier foo as car and another pair -- with identifier bar as car
  * and value nil as cdr -- as cdr. Pairs are pretty-printed when converted to
...
case class SExpPair(car: SExp, cdr: SExp, idn: Identity) extends SExp:
  ...
  def toStringRest: String =
    cdr match
      ...
      case _                       => s"$car . $cdr"
  def subexpressions: List[SExp] = List(car, cdr)

case class SExpVector(elements: List[SExp], idn: Identity) extends SExp:

  override def toString: String =
    s"#(${elements.map(_.toString).mkString(" ")})"
  def subexpressions: List[SExp] = elements
...
case class SExpId(id: Identifier[SExp]) extends SExp:
  val idn: Identity = id.idn
  override def toString: String = id.toString
  def subexpressions: List[SExp] = List()
