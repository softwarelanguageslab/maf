...
/** Trait can be mixed in to obtain access to a "check" function, which checks
  * whether a program is valid according to the specification with regards to
  * the context where internal definitions may occur
  */
trait UndefinerTester:
  ...
  implicit def toResult(b: Boolean)(using allowed: Boolean): Result =
    if b then ErrorNoIdentity else NoError(allowed)

  /** Checks whether the given Scheme program contains a define in the wrong
    * contexT.
    *
    * R5RS specifies that defiens may only occur on a top-level, or in the
    * beginning of a body (so called internal defines).
    *
  ...
  private def checkSequence(s: List[SchemeExp])(allowed: Boolean): Result =
    ...
    s match
      case (SchemeDefineVariable(name, value, idn)) :: rest =>
        if allowed then check(value, false) || checkSequence(rest)(true)
        else Error(idn)
  ...
  protected def check(s: SchemeExp, allowed: Boolean): Result =
    ...
    s match
      ...
      case SchemeLet(bindings, body, idn) =>
        bindings
          .map(_._2)
          .foldLeft[Result](false)(_ || check(_, false)) || checkSequence(body)(
          true
        )
      ...
      case CSchemeFork(body, _) =>
        check(
          body,
          false
        ) // TODO: check if this is correct. Defines allowed at the start of the body
      ...
      case ContractSchemeDepContract(domains, rangeMaker, _) =>
        domains.foldLeft[Result](false)(_ || check(_, false)) || check(
          rangeMaker,
          false
        )
      ...
      case ContractSchemeDefineContract(
        ...
        if allowed then check(contract, false) || check(expression, true)
        else true
      ...
      case ContractSchemeProvide(outs, idn) =>
        false // TODO: actually only allowed on the top-level, but we don't have any information about the level of the definition yet
      ...
      case ASchemeSend(actorRef, _, args, _) =>
        check(actorRef, false) || args.foldLeft[Result](false)(
          _ || check(_, false)
        )
      ...
      case ASchemeActor(_, ASchemeSelect(handlers, _), _, _, _) =>
        handlers.values.foldLeft[Result](false) { case (result, (_, bdy)) =>
          result || check(bdy, true)
        }

      case AContractSchemeMessage(tag, argumentContracts, ensureContract, _) =>
        // Nothing is allowed to have defines in them
        argumentContracts.foldLeft[Result](false) { case (result, contract) =>
          check(contract, false)
        } || check(ensureContract, false)
