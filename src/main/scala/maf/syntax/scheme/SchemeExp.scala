...
object SchemeLambdaExp:
    given Show[SchemeLambdaExp] with
        def show(e: SchemeLambdaExp): String = e.toString()
...
sealed trait SchemeLettishExp extends SchemeExp:
    ...
    def subexpressions: List[Expression[SchemeExp]] =
        bindings.foldLeft(List[Expression[SchemeExp]]())((a, b) => b._2 :: b._1 :: a) ::: body
    def letName: String
    override def prettyString(indent: Int): String =
        val id = letName
...
case class SchemeVarLex(id: Identifier[SchemeExp], lexAddr: LexicalRef) extends SchemeVarExp:
...
case class ContractSchemeFlatContract(expression: SchemeExp, idn: Identity) extends ContractSchemeExp:
...
/** Creates a struct getter can be applied like a function
  ...
  */
case class MakeStructGetter(tag: String, idx: Int, idn: Identity) extends MakeStruct
...
/** Creates a struct setter can be applied like a function
  ...
  */
case class MakeStructSetter(tag: String, idx: Int, idn: Identity) extends MakeStruct

/** Creates a constructor, that can be applied like a function.
  *
  * (define posn (_make_struct_constr 'posn 2)) (posn 10 20)
  */
case class MakeStructConstr(tag: String, siz: Int, idn: Identity) extends MakeStruct
...
case class SchemeSource(name: Identifier[SchemeExp], idn: Identity) extends SchemeExp:
...
case class SchemeSink(name: Identifier[SchemeExp], idn: Identity) extends SchemeExp:
    def fv: Set[String] = Set(name.toString)
    def subexpressions: List[Expression[SchemeExp]] = List(name)
    override val height: Int = name.height + 1
    override def toString: String = s"(sink $name)"

case class SchemeSanitizer(name: Identifier[SchemeExp], idn: Identity) extends SchemeExp:
    def fv: Set[String] = Set(name.toString)
    def subexpressions: List[Expression[SchemeExp]] = List(name)
    override val height: Int = name.height + 1
