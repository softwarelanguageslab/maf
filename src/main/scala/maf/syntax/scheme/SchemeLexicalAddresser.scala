...
enum LexicalRef:
  ...
  case VarRef(id: Identifier[SchemeExp])

trait BaseSchemeLexicalAddresser:

  /** A frame is a mapping from variable names to the lexical definition site */
  type Frame = Map[String, Identifier[SchemeExp]]
  type Scope = List[Frame]

  val emptyFrame: Frame = Map.empty
  val emptyScope: Scope = List.empty

  def lookup(scope: Scope, name: String): Option[Identifier[SchemeExp]] =
    scope match
      case Nil           => None
      case frame :: rest => frame.get(name).orElse(lookup(rest, name))

  def extendScope(scope: Scope, id: Identifier[SchemeExp]): Scope = scope match
    case frame :: _ if frame.contains(id.name) =>
  ...
  case class LexicalEnv(scope: Scope)(globals: String => Boolean):
    ...
    def extend(id: Identifier[SchemeExp]): LexicalEnv =
      LexicalEnv(extendScope(scope, id))(globals)
    def extend(ids: Iterable[Identifier[SchemeExp]]): LexicalEnv =
      ids.foldLeft(this)((acc, idf) => acc.extend(idf))
  ...
  def translateProgram(
      prg: List[SchemeExp],
      globals: String => Boolean = _ => true
  ): List[SchemeExp] =
    translateBody(
      prg,
      initialLEnv(globals).newFrame.extend(SchemeBody.defs(prg))
    )
  ...
  def translate(exp: SchemeExp, lenv: LexicalEnv): SchemeExp = exp match
    ...
    case SchemeLambda(nam, prs, bdy, ann, pos) =>
      SchemeLambda(
        nam,
        prs,
        translateBody(bdy, lenv.newFrame.extend(prs)),
        ann,
        pos
      )
    case SchemeVarArgLambda(nam, prs, vararg, bdy, ann, pos) =>
      SchemeVarArgLambda(
        ...
        prs,
        vararg,
        translateBody(bdy, lenv.newFrame.extend(prs).extend(vararg)),
        ann,
        pos
      )
    case SchemeVar(id) =>
    ...
    case SchemeLetStar(bindings, body, pos) =>
      val (bdsLex, extEnv) =
        bindings
          .foldLeft[(List[(Identifier[SchemeExp], SchemeExp)], LexicalEnv)](
            (Nil, lenv.newFrame)
          ) { case ((accBds, accEnv), (id, vexp)) =>
            ((id, translate(vexp, accEnv)) :: accBds, accEnv.extend(id))
          }
    ...
    case ContractSchemeCheck(contract, valueExpression, idn) =>
      ContractSchemeCheck(
        translate(contract, lenv),
        translate(valueExpression, lenv),
        idn
      )
    case m: MakeStruct =>
      m // no variables inside a makestruct expression so lexical adressing does not need to be recursively called
    case _ => throw new Exception(s"Unsupported Scheme expression: $exp")

  def translateContractSchemeOut(
      outs: List[ContractSchemeProvideOut],
      lenv: LexicalEnv
  ): List[ContractSchemeProvideOut] =
