...
class BaseSchemePrelude:
  ...
  def parseDef(dff: String, nam: String): List[SchemeExp] =
    SchemeParser.parse(dff, Some(nam))

  lazy val primDefsParsed: Map[String, SchemeExp] = primDefs.map {
    case (nam, str) =>
  ...
  /** Transively adds all required definitions to the prelude, in addition to
    * those mentioned in `incl`.
    */
  def addPrelude(
      prg: List[SchemeExp],
      incl: Set[String] = Set.empty
  ): List[SchemeExp] =
