...
case class PtrAddr[Context](exp: SchemeExp, ctx: Context) extends SchemeAddress[Context]:
    ...
    def printable: Boolean = true
case class AddrWrapper[V: Lattice](adr: Address) extends SchemeAddress[Nothing], ValAddress[V]:
    val idn: Identity = adr.idn
    def printable: Boolean = true
case class PrmAddr[V: Lattice](nam: String) extends SchemeAddress[Nothing], ValAddress[V]:
    type Value = V
    def printable = false
