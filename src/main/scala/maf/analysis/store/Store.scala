...
case class BasicStore[A <: Address, V](content: Map[A, V])(using
    lat: LatticeWithAddrs[V, A]
...
case class CountingStore[A <: Address, V](content: Map[A, (V, AbstractCount)])(
    using
    lat: LatticeWithAddrs[V, A],
    shouldCount: A => Boolean
) extends AbstractGC[A]:
...
object CountingStore:
  def empty[A <: Address, V](using
      LatticeWithAddrs[V, A],
      A => Boolean
  ): CountingStore[A, V] = CountingStore(Map.empty)
...
case class LocalStore[A <: Address, V](
...
)(using lat: LatticeWithAddrs[V, A], shouldCount: A => Boolean)
    extends AbstractGC[A]:
  sto =>
  // "get"-functions return an Option
  inline def get(a: A): Option[(V, AbstractCount)] = content.get(a)
  ...
  case class Delta(delta: SmartMap[A, (V, AbstractCount)])
      extends AbstractGC[A]:
...
object LocalStore:
  ...
  def from[A <: Address, V](
      bds: Iterable[(A, V)]
  )(using LatticeWithAddrs[V, A], A => Boolean): LocalStore[A, V] =
