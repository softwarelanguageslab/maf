          ...
          res <- fv match
            ...
            case Value.Primitive(p) =>
              for argsv <- tailcall(evalArgs(args, env, timeout, version))
                yield {
                  Primitives.allPrimitives(p).call(call, args.zip(argsv))
                }
            case v =>
              signalException(ValueNotApplicable(v, idn))
        yield {
          dynAnalysis = dynAnalysis + (e -> (dynAnalysis.getOrElse(e, Set()) + res))
          res
        }
        ...
        for
          ...
          res <- condv match
            case Value.Bool(false) => {
              tailcall(eval(alt, env, timeout, version))
            }
        ...
        env.get(id.name) match
          ...
          case Some(addr) =>
            lookupStoreOption(addr) match
              ...
              case Some(v) => {
                dynAnalysis = dynAnalysis + (varExp -> (dynAnalysis.getOrElse(varExp, Set()) + v))
                done(v)
              }
