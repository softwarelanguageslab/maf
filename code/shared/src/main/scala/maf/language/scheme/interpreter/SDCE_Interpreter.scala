...
class SDCE_Interpreter(cb: (Identity, ConcreteValues.Value) => Unit = (_, _) => (),
                       io: IO = new EmptyIO(),
                       problematicValue: ConcreteValues.Value,
                       soundnessTest: () => Boolean
                      ) extends PreHaltInterpreter(cb, io, problematicValue, soundnessTest):
  ...
  def runAndIdentifyDeadCode(
    ...
    aliveCode = Set()
    (super.run(program, timeout, version), aliveCode)
  ...
  override def eval(e: SchemeExp, env: Env, timeout: Timeout.T, version: Version): TailRec[Value] =
    ...
    e match
      ...
      case another: _ => aliveCode = aliveCode.+(another)
