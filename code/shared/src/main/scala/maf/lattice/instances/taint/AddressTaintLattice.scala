...
object AddressTaintLattice {
  ...
  class AddressTaintLattice extends TaintLattice[TaintSet, T] {
    ...
    override def join(x: T[TaintSet], y: => T[TaintSet]): T[TaintSet] = { case (Taints(x), Taints(y)) => Taints(x.union(y)) }
    override def subsumes(x: T[TaintSet], y: => T[TaintSet]): Boolean = { case (Taints(x), Taints(y)) => y.subsetOf(x) }
    override def eql[B: BoolLattice](x: T[TaintSet], y: T[TaintSet]): B = { case (Taints(x), Taints(y)) => BoolLattice[B].inject(x == y) }
