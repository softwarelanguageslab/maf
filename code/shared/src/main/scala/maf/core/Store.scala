...
case class BasicStore[A <: Address, V](content: Map[A,V], cachedRefs: Map[A,Set[A]])(implicit val lattice: LatticeWithAddrs[V, A]) extends MapStore[A, V] with AbstractGC[A, V] {
