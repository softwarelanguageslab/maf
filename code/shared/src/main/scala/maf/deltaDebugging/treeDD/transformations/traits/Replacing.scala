package maf.deltaDebugging.treeDD.transformations.traits
...
object Replacing:
  ...
  */
  ...
  private def constantLambdas(): List[SchemeVarArgLambda] = List(
    ...
    newConstantLambda(SchemeValue(Value.String("a"), NoCodeIdentity)),
    newConstantLambda(SchemeValue(Value.Symbol("a"), NoCodeIdentity)),
