...
//    def reindex(base: Int, onChange: (SchemeExp, SchemeExp) => Unit = (_, _) => ()): Symbolic =
//        def visit(exp: SchemeExp): SchemeExp = exp match
//            case SchemeFuncall(f, args, idn) =>
//              SchemeFuncall(visit(f), args.map(visit), idn)
//            case orig @ SchemeVar(id) =>
//              val reindexed = Symbolic.reindex(base, id.name)
//              if reindexed != id.name then onChange(orig, SchemeVar(Identifier(reindexed, id.idn)))
//
//              SchemeVar(Identifier(reindexed, id.idn))
//
//            case orig @ SchemeVarLex(id, lexAdr) =>
//              val reindexed = Symbolic.reindex(base, id.name)
//              if reindexed != id.name then onChange(orig, SchemeVarLex(Identifier(reindexed, id.idn), lexAdr))
//              SchemeVarLex(Identifier(reindexed, id.idn), lexAdr)
//
...
//     * a -> x0; b -> x1
//     */
//    def reindex(base: Int): (SymbolicStore, List[SymChange]) =
//        var changes: List[SymChange] = List()
//        def change(original: SchemeExp, nww: SchemeExp): Unit =
//          changes = changes ++ List(SymReplace(Symbolic(original), Symbolic(nww)))
//
//        (SymbolicStore(mapping.mapValues(_.reindex(base, change)).toMap), changes)
//
//    /**
...
//    def gc(pc: List[SchemeExp], keep: List[String]): SymbolicStore =
//        val allSubexpressions = pc.flatMap(_.allSubexpressions)
//        //println(s"filtering $allSubexpressions with $mapping")
//        this.copy(mapping = mapping.filter { case (k, v) => allSubexpressions.contains(v.expr) || keep.contains(k) }.toMap)
//
...
//
//    /** Converts the given list of lists of conjunctions to a Horn clause (i.e., a disjunction of conjunctions) */
//    def toHornClause(clauses: List[List[SchemeExp]]): SchemeExp =
//        val disjunctions = clauses.map(conjunctions => SchemeFuncall(SchemeVar(Identifier("conj/v", Identity.none)), conjunctions, Identity.none))
//        SchemeFuncall(SchemeVar(Identifier("disj/v", Identity.none)), disjunctions, Identity.none)
//
///**
...
//
//        this.copy(pcs = computeMap(newache), cachedPc = newCache)
