...
abstract class SchemeModFLocal(prog: SchemeExp) extends ModAnalysis[SchemeExp](prog)
    ...
    case class IteFrame(csq: Exp, alt: Exp, env: Env) extends Frame
    case class AssFrame(id: Identifier, env: Env) extends Frame
    case class FunFrame(fun: SchemeFuncall, ags: List[Exp], env: Env) extends Frame
    ...
    class SchemeIntraAnalysis(cmp: Component) extends IntraAnalysis(cmp) {
        ...
        private def lookupVariable(id: Identifier, env: Env, sto: Sto)(clb: Address => Unit): Unit = 
        ...
        private def evalVariable(id: Identifier, env: Env, sto: Sto, kon: Kon): Unit = 
            lookupVariable(id, env, sto) { addr => continue(kon, lookupV(sto, addr), sto) }

        private def assignVariable(id: Identifier, env: Env, sto: Sto, vlu: Val, kon: Kon): Unit =
        ...
        private def applyClosures(
            ...
            lattice.getClosures(fvl).foreach {
                case (lam, lex: Env @unchecked) if lam.check(agc) =>
                    ...
                    val sto4 = sto3.extend(KonAddr(lam, cctx), K(kon match {
        ...
        protected def allocCons(pairExp: Exp, sto: Sto, car: Val, cdr: Val): (Val, Sto) =
            allocateVal(pairExp, sto, lattice.cons(car, cdr))
        protected def allocateVal(exp: Exp, sto: Sto, vlu: Val): (Val, Sto) = {
        ...
        private def continue(kon: Kon, vlu: Val, sto: Sto): Unit = kon match {
