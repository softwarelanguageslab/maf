...
trait SchemeSemantics {
  ...
  private def applyFun(cll: Cll, fun: Val, ags: List[(Exp, Val)]): A[Val] =
    mjoin(applyPrimitives(cll, fun, ags), applyClosures(cll, fun, ags))

  private def applyPrimitives(cll: Cll, fun: Val, ags: List[(Exp, Val)]): A[Val] =
    lattice.getPrimitives(fun).foldMapM { prm =>
      applyPrimitive(cll, primitives(prm), ags)
    }

  private def applyPrimitive(cll: Cll, prm: Prim, ags: List[(Exp, Val)]): A[Val] =
    prm.call(cll, ags)

  private def applyClosures(cll: Cll, fun: Val, ags: List[(Exp, Val)]): A[Val] = {
    val agc = ags.length
  ...
  private def applyClosure(cll: Cll, clo: Clo, ags: List[(Exp, Val)]): A[Val] = 
    // TODO: GC here
    for {
  ...
  private def bindArgs(clo: Clo, ags: List[(Exp,Val)], ctx: Ctx): A[Unit] = {
    val (lam, lex: Env @unchecked) = clo
    val (fxa, vra) = ags.splitAt(lam.args.length)
    for {
      _ <- writePar(lam.args.zip(fxa.map(_._2)), ctx) // bind fixed args
      _ <- lam.varArgId match { // bind varargs
        case None => unit(())
        case Some(varArg) => allocLst(vra).flatMap(writePar(varArg, ctx, _))
      }
