...
trait IncrementalGlobalStore[Expr <: Expression] extends IncrementalModAnalysis[Expr] with GlobalStore[Expr] with IncrementalAbstractDomain[Expr] {
   ...
   * @note
   *   We could also store it as R ~> Set[W], but this seems slightly easier (doesn't require a foreach over the set `reads`).
   */
  var addressDependencies: Map[Component, Map[Addr, Set[Addr]]] = Map().withDefaultValue(Map().withDefaultValue(Set()))

  /**
   * Keeps track of all inferred SCCs during an incremental update. To avoid confusion with analysis components, we call these Strongly Connected
   * Addresses (SCC containing addresses).
   */
   ...
   *
   * @note
   *   Also removes possible dependencies on this address!
   */
   ...
   * @return
   *   Returns a boolean indicating whether the address was updated, and hence whether the corresponding dependency should be triggered.
   */
  @nonMonotonicUpdate
  def updateAddrInc(
      cmp: Component,
