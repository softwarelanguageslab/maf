...
/**
  ...
  */
  ...
  /**
    ...
    */
  case class ModuleSummary(content: Map[Component, MultiSet[Dependency]], depCmps: Map[Dependency, Set[Component]], totalDepCount: Int) {
    ...
    def addDependency(cmp: Component, dep: Dependency) =
      ModuleSummary(content + (cmp -> (content(cmp) + dep)),
                    depCmps + (dep -> (depCmps.getOrElse(dep, Set.empty) + cmp)),
                    totalDepCount + 1)
    def clearDependency(dep: Dependency) = {
      val cps = depCmps(dep)
  ...
  private def updateModuleSummary(update: Component => Component)(ms: ModuleSummary): ModuleSummary = {
    ...
    ModuleSummary(updated,
                  updateMap(updateDep(update), updateSet(update))(ms.depCmps),
                  updated.values.map(_.cardinality).sum) 
  }
