...
import maf.TurgutsThesis.gtr.transformations.schemeLambda.RemoveLambdaParamWithDeepDrop
...
class RemoveLambdaParamWithDeepDropTest extends AnyFlatSpecTransformations {
  "RemoveLambdaParamWithDeepDrop" should "deep drop a lambdas param" in {
    ...
    suggestedTrees = RemoveLambdaParamWithDeepDrop.transform(t, defineExp).toList
  ...
  "RemoveLambdaParamWithDeepDrop" should "return empty list given a non-lambda-binding exp" in {
    ...
    suggestedTrees = RemoveLambdaParamWithDeepDrop.transform(t, fAppl).toList
