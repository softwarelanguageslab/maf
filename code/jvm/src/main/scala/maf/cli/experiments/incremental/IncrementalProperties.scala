...
object IncrementalSchemeModXProperties:
    def main(args: IncArgs): Unit =
        if args.typeLattice then
            if args.curated then (new IncrementalSchemeModFTypeProperties).execute(IncrementalSchemeBenchmarkPrograms.sequentialCurated.toArray)
        ...
        if args.cpLattice then
            if args.curated then (new IncrementalSchemeModFCPProperties).execute(IncrementalSchemeBenchmarkPrograms.sequentialCurated.toArray)
