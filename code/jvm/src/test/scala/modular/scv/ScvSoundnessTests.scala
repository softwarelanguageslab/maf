...
class ScvNguyenSoundnessTests extends ScvSoundnessTests:
    ...
    override def benchmarks: Set[String] = SchemeBenchmarkPrograms.scvNguyenBenchmarks -- /* unable to generate inputs --> */ Set(
      "test/scv/NguyenGTH18/safe/softy/subst.rkt",
...
class ScvNguyenSoundnessTestsFunctionSummariesTopSort extends ScvSoundnessTests:
    def analysis(program: SchemeExp): Analysis =
        SchemeAnalyses.scvModAnalysisFunctionSummary(program) //WithSharedPathStore(program)
