; Changes: use vectors instead of lists to represent pts. (The original version "test/R5RS/WeiChenRompf2019/toplas98/nbody-processed.scm" uses vectors -- also added a print to the end of this file.)
...
(define make-raw-pt
  (lambda (pt-1 pt-2 pt-3)
    ((<change> list vector) '<pt> pt-1 pt-2 pt-3)))
(define pt?
  (lambda (obj)
    (if ((<change> list? vector?) obj)
        (if (= ((<change> length vector-length) obj) 4)
            (eq? (<change> (car obj) (vector-ref obj 0)) '<pt>)
            #f)
        #f)))
(define pt-1
  (lambda (obj)
    ...
    (<change> (cadr obj) (vector-ref obj 1))))
(define pt-2
  (lambda (obj)
    (if (pt? obj)
        (void)
        (error 'pt-2 "~s is not a ~s" obj '<pt>))
    (<change> (caddr obj) (vector-ref obj 2))))
(define pt-3
  (lambda (obj)
    (if (pt? obj)
        (void)
        (error 'pt-3 "~s is not a ~s" obj '<pt>))
    (<change> (cadddr obj) (vector-ref obj 3))))
(define set-pt-1!
  (lambda (obj newval)
    (if (pt? obj)
        (void)
        (error
         ...
         obj
         '<pt>))
    (<change> (set-car! (cdr obj) newval) (vector-set! obj 1 newval))))
(define set-pt-2!
  (lambda (obj newval)
    (if (pt? obj)
        (void)
        (error
         ...
         obj
         '<pt>))
    (<change> (set-car! (cddr obj) newval) (vector-set! obj 2 newval))))
(define set-pt-3!
  (lambda (obj newval)
    (if (pt? obj)
        (void)
        (error
         ...
         obj
         '<pt>))
    (<change> (set-car! (cdddr obj) newval) (vector-set! obj 3 newval))))
