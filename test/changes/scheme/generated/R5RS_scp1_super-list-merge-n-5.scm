...
(letrec ((super-merge-n (lambda (lsts n)
                          (letrec ((geef-n+rest (lambda (lst n)
                                                  (if (<change> (let ((__or_res (= 0 n))) __or_res (if __or_res __or_res (null? lst))) (not (let ((__or_res (= 0 n))) __or_res (if __or_res __or_res (null? lst)))))
                                                     (cons () lst)
                             ...
                             (if (null? lsts)
                                ...
                                (let* ((g-n+rest (geef-n+rest (car lsts) n))
                                   ...
                                   (append
                                      first
                                      (super-merge-n
                                         (append (cdr lsts) (if (<change> (null? rest) (not (null? rest))) rest (list rest)))
                                         n))))))))
   (<change>
      (equal?
         (super-merge-n
