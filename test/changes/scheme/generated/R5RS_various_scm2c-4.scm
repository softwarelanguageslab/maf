...
; * added: 13
; * swaps: 3
; * negated predicates: 17
(letrec ((void (lambda ()
                 (if #f #t #f)))
         (tagged-list? (lambda (tag l)
                         (if (<change> (pair? l) (not (pair? l)))
                            (eq? tag (car l))
                            #f)))
         ...
         (gensym (lambda (param)
                   ...
                   (string->symbol
                      (string-append
                         (if (<change> (symbol? param) (not (symbol? param)))
                            (symbol->string param)
                            param)
                         "$"
         ...
         (insert (lambda (sym S)
                   (if (not (pair? S))
                      (list sym)
                      (if (<change> (eq? sym (car S)) (not (eq? sym (car S))))
                         S
         ...
         (remove (lambda (sym S)
                   (<change>
                      ()
                      (display (if (not (eq? (car S) sym)) (cdr S) (cons (car S) (remove sym (cdr S))))))
                   (if (not (pair? S))
                      ()
                      (if (eq? (car S) sym)
                         (cdr S)
         ...
         (substitute-var (lambda (env var)
                           (<change>
                              ()
                              (display (let ((sub (assq var env))) (if sub (cadr sub) var))))
                                           ...
                                           (if (letrec? exp)
                                              ...
                                              (if (begin? exp)
                                                 ...
                                                 (if (cell-get? exp)
                                                    ...
                                                    (if (<change> (cell? exp) (not (cell? exp)))
         ...
         (wrap-mutables (lambda (exp)
                          (letrec ((wrap-mutable-formals (lambda (formals body-exp)
                             ...
                             (if (const? exp)
                                exp
                                (if (ref? exp)
                                   (if (<change> (is-mutable? exp) (not (is-mutable? exp)))
