...
(letrec ((map (lambda (f lst)
         ...
         (singlet? (lambda (list)
                     (<change>
                        ...
                        ((lambda (x) x) (if (pair? list) (null? (cdr list)) #f)))))
         ...
         (macro? (lambda (s-exp)
                   (<change>
                      ...
                      ((lambda (x) x) (three-d-tagged-list? 'macro s-exp)))))
         ...
         (and->if (lambda (exp)
                    (if (<change> (not (and? exp)) (not (not (and? exp))))
                       exp
                 ...
                 (if (symbol? exp)
                    ...
                    (if (<change> (number? exp) (not (number? exp)))
                       ...
                       (if (boolean? exp)
                          ...
                          (if (string? exp)
                             exp
                             (if (<change> (procedure? exp) (not (procedure? exp)))
                                (exp)
                                (if (app? exp)
         ...
         (eval-set! (lambda (exp env)
                      ...
                      (<change>
                         ...
                         ((lambda (x) x) (env-set! env (set!->var exp) (eval (set!->exp exp) env))))))
         ...
         (env-extend (lambda (env var value)
                       ...
                       (lambda (seek-var modify? value!)
                          (<change>
                             ...
                             ((lambda (x) x)
                                (if (<change> (eq? var seek-var) (not (eq? var seek-var)))
                                   (if modify?
                                      (<change>
                                         (set! value value!)
                                         value)
         ...
         (empty-env (lambda (var modify? value!)
                      ...
                      (<change>
                         ...
                         (if modify?
                            ...
                            (begin
                               (display var)
                               ((lambda (x) x) (display "error: cannot look up undefined variable: "))
                               (display var))))
                      (<change>
                         (error)
                         ((lambda (x) x) (error)))))
