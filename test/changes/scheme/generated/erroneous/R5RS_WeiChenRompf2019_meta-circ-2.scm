...
(letrec ((map (lambda (f lst)
         ...
         (begin? (lambda (s-exp)
                   (<change>
                      ...
                      ((lambda (x) x) (three-d-tagged-list? 'begin s-exp)))))
         ...
         (cond? (lambda (s-exp)
                  (<change>
                     ...
                     ((lambda (x) x) (three-d-tagged-list? 'cond s-exp)))))
         ...
         (letrec->lets+sets (lambda (exp)
                              (<change>
                                 ()
                                 (append sets (letrec->body exp)))
                              (if (<change> (not (letrec? exp)) (not (not (letrec? exp))))
                                 exp
         ...
         (cond->if (lambda (cond-exp)
                     (<change>
                        ...
                        ((lambda (x) x) (if (not (cond? cond-exp)) cond-exp (cond-clauses->if (cond->clauses cond-exp)))))))
         ...
         (eval-cond (lambda (exp env)
                      (<change>
                         ...
                         ((lambda (x) x) (eval (cond->if exp) env)))))
         (eval-and (lambda (exp env)
                     (<change>
                        (eval (and->if exp) env)
                        ((lambda (x) x) (eval (and->if exp) env)))))
                          ...
                          (<change>
                             ...
                             ((lambda (x) x)
                                (let ((args (app->args app-exp)))
                                   (if (macro? fun)
                                      (eval (apply (macro->proc fun) args) env)
                                      (if (<change> (syntax-primitive? fun) (not (syntax-primitive? fun)))
                                         ((syntax-primitive->eval fun) app-exp env)
                                         (let ((arg-values (eval* args env)))
                                            (apply fun arg-values))))))))))
   (eval
