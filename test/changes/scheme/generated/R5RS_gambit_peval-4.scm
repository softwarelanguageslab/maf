...
(letrec ((every? (lambda (pred? l)
         ...
         (binding-frame (lambda (var env)
                          (if (null? env)
                             #f
                             (if (<change> (let ((__or_res (eq? (caar env) 'let))) ((lambda (x) x) (if __or_res (eq? (caar env) 'letrec) __or_res))) (not (let ((__or_res (eq? (caar env) 'let))) ((lambda (x) x) (if __or_res (eq? (caar env) 'letrec) __or_res)))))
         ...
         (bound-expr (lambda (var frame)
                       (if (let ((__or_res (eq? (car frame) 'let))) (<change> (if __or_res __or_res (eq? (car frame) 'letrec)) ((lambda (x) x) (if __or_res __or_res (eq? (car frame) 'letrec)))))
         ...
         (try-peval (lambda (proc args)
                      (<change>
                         ()
                         (display partial-evaluate))
                      (<change>
                         (partial-evaluate proc args)
                         ((lambda (x) x) (partial-evaluate proc args)))))
