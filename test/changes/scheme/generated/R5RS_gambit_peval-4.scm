...
(letrec ((every? (lambda (pred? l)
         ...
         (alphatize (lambda (exp env)
                      (letrec ((alpha (lambda (exp)
                                        (if (const-expr? exp)
                                           ...
                                           (if (<change> (symbol? exp) (not (symbol? exp)))
                                              (let ((x (assq exp env)))
                                                 (if x (cdr x) exp))
         ...
         (const-value (lambda (expr)
                        (<change>
                           ...
                           ((lambda (x) x) (if (pair? expr) (cadr expr) expr)))))
         (quot (lambda (val)
                 (list 'quote val)))
         (new-variables (lambda (parms env)
                          (<change>
                             (append (map (lambda (x) (cons x (new-variable x))) parms) env)
                             ((lambda (x) x) (append (map (lambda (x) (cons x (new-variable x))) parms) env)))))
         (*current-num* 0)
                                                                                  ...
                                                                                  (if (eq? (car exp) 'begin)
                                                                                     (begin
                                                                                        ...
                                                                                        (if (null? (cddr exp))
                                                                                           (set-car! where (cadr exp))
                                                                                           #f))
                                                                                     (if (let ((__or_res (eq? (car exp) 'let))) ((lambda (x) x) (if __or_res __or_res (eq? (car exp) 'letrec))))
                                                                                        (let ((new-env (cons exp env)))
                                                                                           (letrec ((keep (lambda (i)
                                                                                                            (if (>= i (length (cadar where)))
                                                                                                               ()
                            ...
                            (lambda (args)
                               (if (= (length args) 1)
                                  (if (pair? (car args))
                                     ...
                                     #f)
                                  #f)))
                         (cons '+ (lambda (args) (<change> () every?) (if (every? number? args) (quot (sum args 0)) #f)))
                         (cons
                            '*
                            (lambda (args)
