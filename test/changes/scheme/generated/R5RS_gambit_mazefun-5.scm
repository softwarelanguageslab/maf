...
(letrec ((foldr (lambda (f base lst)
         ...
         (matrix-map (lambda (f mat)
                       (map (lambda (lst) (<change> () lst) (<change> () map) (map f lst)) mat)))
         (initial-random 0)
                         ...
                         (let ((cave (make-matrix
                               ...
                               (possible-holes (concat
                                                 (for
                                                    ...
                                                    (lambda (i)
                                                       (concat
                                                          (for 0 m (lambda (j) (<change> () even?) (if (equal? (even? i) (even? j)) () (list (cons i j)))))))))))
         ...
         (pierce-randomly (lambda (possible-holes cave)
                            (if (<change> (null? possible-holes) (not (null? possible-holes)))
                               cave
                                 ...
                                 (let ((size (matrix-size cave)))
                                    (let ((n (car size))
                                       ...
                                       (let ((i (car pos))
                                             (j (cdr pos)))
                                          (append
                                             (if (<change> (if (> i 0) (matrix-read cave (- i 1) j) #f) (not (if (> i 0) (matrix-read cave (- i 1) j) #f)))
                                                (list (cons (- i 1) j))
                                                ())
                                             (append
                                                ...
                                                (append
                                                   (if (if (> j 0) (matrix-read cave i (- j 1)) #f)
                                                      (list (cons i (- j 1)))
                                                      ())
                                                   (if (<change> (if (< j (- m 1)) (matrix-read cave i (+ j 1)) #f) (not (if (< j (- m 1)) (matrix-read cave i (+ j 1)) #f)))
                                                      (list (cons i (+ j 1)))
                                                      ())))))))))
