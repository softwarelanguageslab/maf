...
(letrec ((foldr (lambda (f base lst)
                ...
                (letrec ((for-aux (lambda (lo)
                                    (<change>
                                       (if (< lo hi)
                                          ...
                                          ())
                                       ((lambda (x) x) (if (< lo hi) (cons (f lo) (for-aux (+ lo 1))) ()))))))
                   (for-aux lo))))
         (concat (lambda (lists)
                   (foldr append () lists)))
         ...
         (matrix-map (lambda (f mat)
                       (map (lambda (lst) (<change> (map f lst) ((lambda (x) x) (map f lst)))) mat)))
         (initial-random 0)
         ...
         (make-maze (lambda (n m)
                      (if (not (if (odd? n) (odd? m) #f))
                         'error
                         (let ((cave (make-matrix n m (lambda (i j) (<change> () even?) (if (if (even? i) (even? j) #f) (cons i j) #f))))
