...
(letrec ((true #t)
                      ...
                      (if (thunk? obj)
                         (let ((result (actual-value (thunk-exp obj) (thunk-env obj))))
                            (<change>
                               ...
                               ((lambda (x) x) (set-car! obj 'evaluated-thunk)))
                 ...
                 (<change>
                    ...
                    ((lambda (x) x)
                       (if (self-evaluating? exp)
                          ...
                          (if (variable? exp)
                             (lookup-variable-value exp env)
                             (if (let ((__or_res (quoted? exp))) (<change> (if __or_res __or_res (quasiquoted? exp)) ((lambda (x) x) (if __or_res __or_res (quasiquoted? exp)))))
                                (eval-quotation-args (text-of-quotation exp) env)
                                (if (assignment? exp)
                                   (eval-assignment exp env)
                                   (if (definition? exp)
   ...
   (<change>
      ...
      ((lambda (x) x) (display "Loading definitions of cons, car, cdr as compound procedures. ")))
   (eval
      (__toplevel_cons
         'begin
         (__toplevel_cons
