                 ...
                 (<change>
                    (if (pair? l)
                       (if (eq? x (car l))
                          ...
                          (cons x (remq x (cdr l))))
                       l)
                    ((lambda (x) x) (if (pair? l) (if (eq? x (car l)) (remq x (cdr l)) (cons x (remq x (cdr l)))) l)))))
                                                                                          ...
                                                                                          (if (eq? msg 'draw-line)
                                                                                             (if (not (eq? msg 'draw-ellipse))
                                                                                                (lambda (x-15 x-16 x-17 x-18 x-19)
                                                                                                   ((lambda (x) x) (draw-ellipse x-15 x-16 x-17 x-18 x-19)))
                                                                                                (if (eq? msg 'draw-text)
                                  ...
                                  (letrec ((set-x! (lambda (new_x)
                                           ...
                                           (draw-line (lambda (x y w h width color)
                                                        (for-each
                                                           (lambda (tile)
                                                              (<change>
                                                                 ((tile 'draw-line) x y w h width color)
                                                                 ((lambda (x) x) ((tile 'draw-line) x y w h width color))))
                                                           tiles)
                                                        (update-callback)))
                                           (clear (lambda ()
                                                    (<change>
         ...
         (four-in-a-row (lambda (width height rows cols title)
                          (let* ((window (make-window width height title))
                             ...
                             (letrec ((switch (lambda ()
                                      ...
                                      (drop-coin (lambda ()
                                                   (if (if (<change> (not (eq? ((game-board 'get) 0 3) 0)) (not (not (eq? ((game-board 'get) 0 3) 0)))) (not (eq? ((game-board 'get) 1 3) 0)) #f)
                                      ...
                                      (start (lambda ()
                                               (letrec ((main-loop (lambda (main-loop-n)
                                                  ...
                                                  (<change>
                                                     ...
                                                     ((lambda (x) x) ((window 'set-background!) "white")))
