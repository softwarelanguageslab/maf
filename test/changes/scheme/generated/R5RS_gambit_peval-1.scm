...
(letrec ((every? (lambda (pred? l)
                         ...
                         (letrec ((rc (lambda (exp ae)
                                        (let ((__cond-empty-body (const-expr? exp)))
                                           (if __cond-empty-body
                                              __cond-empty-body
                                              (if (symbol? exp)
                                                 (if (<change> (eq? exp var) (not (eq? exp var)))
                                                    (begin
                                                       (<change>
                                                          (set! total (+ total 1))
         ...
         (binding-frame (lambda (var env)
                          (if (null? env)
                             #f
                             (if (let ((__or_res (eq? (caar env) 'let))) (<change> () env) (if __or_res __or_res (eq? (caar env) 'letrec)))
         ...
         (bound-expr (lambda (var frame)
                       (if (<change> (let ((__or_res (eq? (car frame) 'let))) (if __or_res __or_res (eq? (car frame) 'letrec))) (not (let ((__or_res (eq? (car frame) 'let))) (if __or_res __or_res (eq? (car frame) 'letrec)))))
         ...
         (constant-fold-global (lambda (name exprs)
                                 (letrec ((flatten (lambda (args op)
                                    ...
                                    (let ((args (if (<change> (let ((__or_res (eq? name '+))) (if __or_res __or_res (eq? name '*))) (not (let ((__or_res (eq? name '+))) (if __or_res __or_res (eq? name '*)))))
                                                  (flatten exprs name)
                                                  exprs)))
                                                   ...
                                                   (if (eq? name '+)
                                                      ...
                                                      (if (eq? name '*)
                                                         (let ((x (reduce-global '* const)))
                                                            (if x
                                                               (let ((y (const-value x)))
                                                                  (cons '* (if (<change> (= y 1) (not (= y 1))) non-const (cons x non-const))))
                                                               (cons name args)))
                                                         (if (eq? name 'cons)
