...
(letrec ((void (lambda ()
         ...
         (cell-get? (lambda (exp)
                      (<change>
                         ...
                         ((lambda (x) x) (tagged-list? 'cell-get exp)))))
                                  ...
                                  (if (if? exp)
                                     ...
                                     (if (set!? exp)
                                        ...
                                        (if (let? exp)
                                           ...
                                           (if (<change> (letrec? exp) (not (letrec? exp)))
                                              not-handled
                                              (if (begin? exp)
         ...
         (c-compile-args (lambda (args append-preamble)
                           (if (not (pair? args))
                              ...
                              (string-append
                                 ...
                                 (if (<change> (pair? (cdr args)) (not (pair? (cdr args))))
         ...
         (c-compile-and-emit (lambda (emit input-program)
                               (letrec ((compiled-program ""))
                                  ...
                                  (<change>
                                     ()
                                     (display (c-compile-program input-program)))
                                  (emit "")
                                  (emit
                                     "
Value __sum ;
