...
(letrec ((void (lambda ()
         ...
         (azip (lambda (list1 list2)
                 (if (if (<change> (pair? list1) (not (pair? list1))) (pair? list2) #f)
         ...
         (substitute (lambda (env exp)
                       (letrec ((substitute-with (lambda (env)
                                                   (<change>
                                                      (lambda (exp)
                                                         (substitute env exp))
                                                      ((lambda (x) x) (lambda (exp) (substitute env exp)))))))
                          (if (null? env)
                             exp
                             (if (const? exp)
                                exp
         ...
         (letrec=>lets+sets (lambda (exp)
                              (if (letrec? exp)
                                 (let* ((bindings (letrec->bindings exp))
                                        ...
                                        (sets (map (lambda (binding) (<change> () cons) (cons 'set! binding)) bindings))
         ...
         (begin=>let (lambda (exp)
                       (letrec ((singlet? (lambda (l)
                                ...
                                (dummy-bind (lambda (exps)
                                              (if (singlet? exps)
                                                 (car exps)
                                                 (if (<change> (pair? exps) (not (pair? exps)))
                                                    (__toplevel_cons
                                                       'let
                                                       (__toplevel_cons
                                                          ...
                                                          (if (cell? exp)
                                                             ...
                                                             (if (set-cell!? exp)
                                                                ...
                                                                (if (closure? exp)
                                                                   ...
                                                                   (if (env-make? exp)
                                                                      (reduce union (map free-vars (env-make->values exp)) ())
                                                                      (if (<change> (env-get? exp) (not (env-get? exp)))
                                                                         (free-vars (env-get->env exp))
                                                                         (if (app? exp)
                                                                            (reduce union (map free-vars exp) ())
                                                                            (error "unknown expression: " exp)))))))))))))))))))))
                                                  ...
                                                  (if (set!? exp)
                                                     (begin
                                                        ...
                                                        (<change>
                                                           ...
                                                           ((lambda (x) x) (analyze-mutable-variables (set!->exp exp)))))
                                                     (if (if? exp)
                                                        (begin
                                                           ...
                                                           (if (letrec? exp)
                                                              (begin
                                                                 ...
                                                                 (<change>
                                                                    ...
                                                                    ((lambda (x) x) (analyze-mutable-variables (letrec->exp exp)))))
                                                              (if (begin? exp)
                                                                 (<change>
                                                                    (begin
                   ...
                   (<change>
                      ...
                      ((lambda (x) x)
                         (letrec ((m (lambda (chars)
                            ...
                            (<change>
                               (list->string (m (string->list (symbol->string symbol))))
                               ((lambda (x) x) (list->string (m (string->list (symbol->string symbol)))))))))))
         (num-environments 0)
         (environments ())
         ...
         (c-compile-and-emit (lambda (emit input-program)
                               (letrec ((compiled-program ""))
                                  ...
                                  (<change>
                                     ...
                                     ((lambda (x) x) (emit "#include <stdlib.h>")))
                                  (<change>
