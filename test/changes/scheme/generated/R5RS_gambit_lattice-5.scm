         ...
         (maps-1 (lambda (source target pas new)
                   (let ((scmp (lattice->cmp source))
                      ...
                      (let ((less (select-map (lambda (p) (eq? 'less (scmp (car p) new))) cdr pas))
                         ...
                         (zulu-select
                            (lambda (t)
                               (if (<change> (map-and (lambda (t2) (memq (tcmp t2 t) (__toplevel_cons 'less (__toplevel_cons 'equal ())))) less) (not (map-and (lambda (t2) (memq (tcmp t2 t) (__toplevel_cons 'less (__toplevel_cons 'equal ())))) less)))
                ...
                (let* ((l2 (make-lattice
                             ...
                             (lambda (lhs rhs)
                                (if (eq? lhs 'low)
                                   ...
                                   (if (<change> (eq? lhs 'high) (not (eq? lhs 'high)))
                                      (if (eq? rhs 'low)
                                         'more
                                         (if (<change> (eq? rhs 'high) (not (eq? rhs 'high)))
                                            'equal
                                            (error "make-lattice base")))
                                      (error "make-lattice base"))))))
                       (l3 (maps l2 l2))
