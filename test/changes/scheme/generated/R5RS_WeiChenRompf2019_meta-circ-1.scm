...
(letrec ((map (lambda (f lst)
         ...
         (if-single? (lambda (s-exp)
                       (<change>
                          ()
                          #f)
                       (if (<change> (if? s-exp) (not (if? s-exp)))
                          (null? (cdddr s-exp))
                          #f)))
         ...
         (make-or (lambda (exps)
                    (<change>
                       ...
                       ((lambda (x) x) (if (null? exps) #f (if (singlet? exps) (car exps) (cons 'or exps)))))))
         (and? (lambda (s-exp)
         ...
         (cond-clause->exp (lambda (clause)
                             (if (singlet? clause)
                                ...
                                (if (else-clause? clause)
                                   (cadr clause)
                                   (if (<change> (arrow-clause? clause) (not (arrow-clause? clause)))
                                      (caddr clause)
         ...
         (eval-letrec (lambda (exp env)
                        (<change>
                           ...
                           ((lambda (x) x) (eval (letrec->lets+sets exp) env)))))
