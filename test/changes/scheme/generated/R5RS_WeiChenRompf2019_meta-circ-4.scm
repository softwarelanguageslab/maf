...
(letrec ((map (lambda (f lst)
                (<change>
                   (if (pair? lst)
                      ...
                      ())
                   ((lambda (x) x) (if (pair? lst) (cons (f (car lst)) (map f (cdr lst))) ())))))
         ...
         (tagged-list? (lambda (tag lst)
                         (if (<change> (pair? lst) (not (pair? lst)))
                            (eq? (car lst) tag)
                            #f)))
         (singlet? (lambda (list)
                     (<change>
         ...
         (define-var? (lambda (s-exp)
                        (<change>
                           ...
                           ((lambda (x) x) (if (define? s-exp) (symbol? (cadr s-exp)) #f)))))
         ...
         (lambda-multi? (lambda (s-exp)
                          (<change>
                             ...
                             ((lambda (x) x) (if (<change> (lambda? s-exp) (not (lambda? s-exp))) (symbol? (cadr s-exp)) #f)))))
         ...
         (lambda->body-as-exp (lambda (s-exp)
                                (<change>
                                   ...
                                   ((lambda (x) x) (make-begin (cddr s-exp))))))
         (begin? (lambda (s-exp)
         ...
         (letrec? (lambda (s-exp)
                    (<change>
                       ...
                       ((lambda (x) x) (three-d-tagged-list? 'letrec s-exp)))))
         ...
         (cond-clause->test (lambda (clause)
                              (if (<change> (singlet? clause) (not (singlet? clause)))
                                 (car clause)
                                 (if (else-clause? clause)
                                    #t
                                    (if (arrow-clause? clause)
         ...
         (syntax-primitive? (lambda (value)
                              (<change>
                                 ...
                                 ((lambda (x) x) (three-d-tagged-list? 'syntax-primitive value)))))
         ...
         (letrec->lets+sets (lambda (exp)
                              (if (not (letrec? exp))
                                 exp
                                 (let ((bindings (map (lambda (binding) (list (binding->var binding) #f)) (letrec->bindings exp)))
                                       (sets (map
                                               (lambda (binding)
                                    ...
                                    (<change>
                                       ...
                                       ((lambda (x) x) (list three-d-let bindings (make-begin (append sets (letrec->body exp))))))))))
         ...
         (let*-bindings->let (lambda (bindings body)
                               ...
                               (<change>
                                  ()
                                  (display (cons (list (car bindings)) (list (let*-bindings->let (cdr bindings) body)))))
                               (if (singlet? bindings)
