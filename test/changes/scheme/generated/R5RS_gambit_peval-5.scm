...
(letrec ((every? (lambda (pred? l)
                                 ...
                                 (lambda (x y)
                                    (<change>
                                       (if (not-constant? y)
                                          (__toplevel_cons () ())
                                          (cons x (quot y)))
                                       ((lambda (x) x) (if (not-constant? y) (__toplevel_cons () ()) (cons x (quot y))))))
                                 parms
                                 args)))))))
         (not-constant (list '?))
                                                                                         ...
                                                                                         (if __cond-empty-body
                                                                                            (<change>
                                                                                               ...
                                                                                               (if (eq? (car exp) 'if)
                                                                                                  ...
                                                                                                  (if (eq? (car exp) 'begin)
                                                                                                     (for-each! rec! (cdr exp))
                                                                                                     (if (let ((__or_res (eq? (car exp) 'let))) car (if __or_res __or_res (eq? (car exp) 'letrec)))
                                                                                                        (if (eq? (car exp) 'lambda)
                                                                                                           (rec! (cddr exp))
                                                                                                           (begin
                                                                                                              (for-each! rec! (cdr exp))
                                                                                                              (if (if (null? (cdr exp)) (symbol? (car exp)) #f)
                                                                                                                 (let ((frame (binding-frame (car exp) env)))
                                                                                                                    (if frame
                                                                                                                       (let ((proc (bound-expr (car exp) frame)))
                                                                                                                          ((lambda (x) x)
                                                                                                                             (if (not (if (pair? proc) (eq? (car proc) 'lambda) #f))
                                                                                                                                (begin
                                                                                                                                   (set! changed? #t)
                                                                                                                                   (set-car! where (caddr proc)))
                                                                                                                                #f)))
         ...
         (bound-expr (lambda (var frame)
                       (if (<change> (let ((__or_res (eq? (car frame) 'let))) (if __or_res (eq? (car frame) 'letrec) __or_res)) (not (let ((__or_res (eq? (car frame) 'let))) (if __or_res (eq? (car frame) 'letrec) __or_res))))
                                                         ...
                                                         (if (eq? name '+)
                                                            ...
                                                            (if (eq? name '*)
                                                               ...
                                                               (if (eq? name 'cons)
                                                                  (if (if (const-expr? (cadr args)) (null? (const-value (cadr args))) #f)
                                                                     (list 'list (car args))
                                                                     (if (if (pair? (cadr args)) (<change> (eq? (car (cadr args)) 'list) #f) (<change> #f (eq? (car (cadr args)) 'list)))
                                                                        (<change>
                                                                           (cons 'list (cons (car args) (cdr (cadr args))))
                                                                           (cons name args))
                                                                        (<change>
