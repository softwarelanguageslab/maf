                         ...
                         (letrec ((lex-fixed (lambda (fixed lhs1 rhs1)
                                               (letrec ((check (lambda (lhs2 rhs2)
                                                                 (if (null? lhs2)
                                                                    fixed
                                                                    (let ((probe (base (car lhs2) (car rhs2))))
                                                                       (if (let ((__or_res (eq? probe 'equal))) (if (<change> __or_res (not __or_res)) __or_res (eq? probe fixed)))
                                                                          (check (cdr lhs2) (cdr rhs2))
                                                                          'uncomparable))))))
                                                  (check lhs1 rhs1))))
                                  (lex-first (lambda (lhs3 rhs3)
                       ...
                       (letrec ((select-a-2 (lambda (ac2 lst5)
                                              (if (null? lst5)
                                                 ...
                                                 (select-a-2
                                                    (let ((head2 (car lst5)))
                                                       (<change>
                                                          (if (test2 head2) (cons (func head2) ac2) ac2)
                                                          ((lambda (x) x) (if (test2 head2) (cons (func head2) ac2) ac2))))
                                                    (cdr lst5))))))
                          (<change>
                             (select-a-2 () lst4)
                             ((lambda (x) x) (select-a-2 () lst4))))))
                                                ...
                                                (let ((rest (cdr lst7)))
                                                   (<change>
                                                      (if (null? rest)
                                                         (proc (car lst7))
                                                         (if (proc (car lst7)) (drudge rest) #f))
                                                      ((lambda (x) x) (if (null? rest) (proc (car lst7)) (if (proc (car lst7)) (drudge rest) #f))))))))
                                (<change>
                                   (drudge lst6)
                                   ((lambda (x) x) (drudge lst6)))))))))
         (maps-1 (lambda (source target pas new)
                   (let ((scmp (lattice->cmp source))
                      ...
                      (let ((less (select-map (lambda (p1) (eq? 'less (scmp (car p1) new))) (lambda (l4) (cdr l4)) pas))
                         ...
                         (zulu-select
                            (lambda (t)
                               (if (<change> (map-and (lambda (t2-1) (memq (tcmp t2-1 t) (__toplevel_cons 'less (__toplevel_cons 'equal ())))) less) (not (map-and (lambda (t2-1) (memq (tcmp t2-1 t) (__toplevel_cons 'less (__toplevel_cons 'equal ())))) less)))
                                  (map-and
                                     (lambda (t2-2)
