                      ...
                      (let* ((x 0)
                         ...
                         (letrec ((clear (lambda ()
                                  ...
                                  (draw-text (lambda (text fontsize x y color)
                                               (if (<change> (string? color) (not (string? color)))
                                                  (set! color "")
                                                  #f)
                                               (update-callback)))
                                  ...
                                  (rotate (lambda (tempr)
                                            (set! rotation tempr)
                                            (if (not (let ((__or_res (eq? 90 tempr))) (if __or_res __or_res (let ((__or_res (eq? 0 tempr))) (if __or_res __or_res (let ((__or_res (eq? 180 tempr))) (if (<change> __or_res (not __or_res)) __or_res (eq? 270 tempr))))))))
                                               (begin
