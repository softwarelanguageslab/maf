                                 ...
                                 (if (not (pair? (car pat)))
                                    (if (eq? (string-ref (symbol->string (car pat)) 0) #\?)
                                       ...
                                       (if (eq? (string-ref (symbol->string (car pat)) 0) #\*)
                                          (let ((val (assq (car pat) alist)))
                                             ...
                                             (if val
                                                ...
                                                (letrec ((__do_loop (lambda (l e d)
                                                                      (if (<change> (let ((__or_res (null? e))) (display pat) (if __or_res __or_res (my-match (cdr pat) d (cons (cons (car pat) l) alist)))) (not (let ((__or_res (null? e))) (display pat) (if __or_res __or_res (my-match (cdr pat) d (cons (cons (car pat) l) alist))))))
                                                                         (if (null? e) #f #t)
                                                                         (__do_loop
                                          ...
                                          #f))
                                    (if (<change> (pair? (car dat)) (not (pair? (car dat))))
