...
(letrec ((down-key 0)
                                    ...
                                    (add-layer (lambda ()
                                                 (letrec ((layer (make-layer w h)))
                                                    (<change>
                                                       ...
                                                       ((lambda (x) x) (set! layers (append layers (cons layer ())))))
                                                    layer)))
         ...
         (make-tile (lambda (w h bitmap mask)
                      ...
                      (let* ((x 0)
                         ...
                         (letrec ((clear (lambda ()
                                  ...
                                  (rotate (lambda (tempr)
                                            (set! rotation tempr)
                                            (if (not (let ((__or_res (eq? 90 tempr))) (if __or_res __or_res (let ((__or_res (eq? 0 tempr))) (if __or_res (<change> __or_res (let ((__or_res (eq? 180 tempr))) (if __or_res __or_res (eq? 270 tempr)))) (<change> (let ((__or_res (eq? 180 tempr))) (if __or_res __or_res (eq? 270 tempr))) __or_res))))))
                                               (begin
                                                                        ...
                                                                        (if (eq? msg 'get-w)
                                                                           ...
                                                                           (if (eq? msg 'get-h)
                                                                              ...
                                                                              (if (eq? msg 'rotate-clockwise)
                                                                                 (if (not (eq? msg 'rotate-counterclockwise))
                                                                                    (lambda ()
                                                                                       ...
                                                                                       (if (eq? msg 'draw-rectangle)
                                                                                          ...
                                                                                          (if (eq? msg 'draw-line)
                                                                                             (lambda (x-10 x-11 x-12 x-13 x-14)
                                                                                                ((lambda (x) x) (draw-line x-10 x-11 x-12 x-13 x-14)))
                          ...
                          (letrec ((redraw (lambda ()
                                   ...
                                   (remove-drawable (lambda (drawable)
                                                      (<change>
                                                         ...
                                                         ((lambda (x) x) ((drawable 'set-on-update!) (lambda () #t))))
         ...
         (arrow (lambda (min max draw-adt)
                  ...
                  (<change>
                     ...
                     ((lambda (x) x) (set! global-current min)))))
         (arrow-draw (lambda (arrow-draw-arrow)
                       (<change>
                          ((global-draw-adt 'draw-pointer) #f)
                          ((lambda (x) x) ((global-draw-adt 'draw-pointer) #f)))))
                             ...
                             (letrec ((c2p-w (lambda (x)
                                      ...
                                      (update-coin! (lambda (coin tile)
                                                      (<change>
                                                         ...
                                                         ((lambda (x) x) ((tile 'set-x!) (c2p-w (coin 'get-x)))))
                                      ...
                                      (start (lambda ()
                                               (letrec ((main-loop (lambda (main-loop-n)
                                                  ...
                                                  (<change>
                                                     ...
                                                     ((lambda (x) x) ((window 'set-key-callback!) on-key)))
                                                  ((window 'set-background!) "white")
                                                  (<change>
                                                     (main-loop MAX_LOOP_ITERATIONS)
                                                     ((lambda (x) x) (main-loop MAX_LOOP_ITERATIONS))))))
                                      (dispatch (lambda (msg)
                                                  (<change>
                                                     (if (eq? msg 'start!) (start) #f)
                                                     ((lambda (x) x) (if (eq? msg 'start!) (start) #f))))))
                                (<change>
                                   ()
                                   dispatch)
                                dispatch))))
