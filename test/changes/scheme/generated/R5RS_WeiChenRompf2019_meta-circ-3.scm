...
(letrec ((map (lambda (f lst)
         ...
         (lambda-multi? (lambda (s-exp)
                          (if (<change> (lambda? s-exp) (not (lambda? s-exp)))
                             (symbol? (cadr s-exp))
                             #f)))
         ...
         (let->body-as-exp (lambda (s-exp)
                             (<change>
                                ...
                                ((lambda (x) x) (make-begin (let->body s-exp))))))
         (let*? (lambda (s-exp)
                  (<change>
                     (three-d-tagged-list? 'let* s-exp)
                     ((lambda (x) x) (three-d-tagged-list? 'let* s-exp)))))
         (let*->bindings (lambda (s-exp)
                           (<change>
                              (cadr s-exp)
                              ((lambda (x) x) (cadr s-exp)))))
         (let*->body (lambda (s-exp)
                       (cddr s-exp)))
         (let*->body-as-exp (lambda (s-exp)
                              (make-begin (let*->body s-exp))))
         ...
         (let*->let (lambda (exp)
                      (<change>
                         ()
                         (display (if (not (let*? exp)) exp (let*-bindings->let (let*->bindings exp) (let*->body exp)))))
                      (if (not (let*? exp))
                         exp
         ...
         (eval* (lambda (exps env)
                  ...
                  (<change>
                     ...
                     ((lambda (x) x) (map (eval-with env) exps)))))
