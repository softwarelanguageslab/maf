...
(letrec ((every? (lambda (pred? l)
                                      ...
                                      (if (const-expr? exp)
                                         ...
                                         (if (symbol? exp)
                                            ...
                                            (if (<change> (let ((__or_res (eq? (car exp) 'if))) (if __or_res __or_res (eq? (car exp) 'begin))) (not (let ((__or_res (eq? (car exp) 'if))) (if __or_res __or_res (eq? (car exp) 'begin)))))
                                               ...
                                               (if (let ((__or_res (eq? (car exp) 'let))) (<change> () __or_res) (if __or_res __or_res (eq? (car exp) 'letrec)))
                                                                                           ...
                                                                                           (let ((frame (binding-frame (car exp) env)))
                                                                                              (if frame
                                                                                                 (let ((proc (bound-expr (car exp) frame)))
                                                                                                    (if (not (if (pair? proc) (if (eq? (car proc) 'lambda) (some? const-expr? (cdr exp)) #f) #f))
         ...
         (arg-pattern (lambda (exps)
                        (if (<change> (null? exps) (not (null? exps)))
                           ()
         ...
         (*primitives* (list
                         ...
                         (cons
                            ...
                            (lambda (args)
                               (if (> (length args) 1)
                                  (if (<change> (every? number? args) (not (every? number? args)))
                                     (quot
         ...
         (constant-fold-global (lambda (name exprs)
                                 (letrec ((flatten (lambda (args op)
                                                     (if (null? args)
                                                        ()
                                                        (if (if (<change> (pair? (car args)) (not (pair? (car args)))) (eq? (caar args) op) #f)
                                                           (append (flatten (cdar args) op) (flatten (cdr args) op))
                                                           (cons (car args) (flatten (cdr args) op)))))))
                                    (let ((args (if (<change> (let ((__or_res (eq? name '+))) name (if __or_res __or_res (eq? name '*))) (not (let ((__or_res (eq? name '+))) name (if __or_res __or_res (eq? name '*)))))
                                                  (flatten exprs name)
                                                  exprs)))
