...
(letrec ((lookup (lambda (key table)
         ...
         (my-match (lambda (pat dat alist)
                     (if (null? pat)
                        ...
                        (if (null? dat)
                           ()
                           (if (<change> (let ((__or_res (eq? (car pat) '?))) (if (not __or_res) __or_res (eq? (car pat) (car dat)))) (not (let ((__or_res (eq? (car pat) '?))) (if (not __or_res) __or_res (eq? (car pat) (car dat))))))
                                          ...
                                          (let ((val (assq (car pat) alist)))
                                             (if val
                                                ...
                                                (letrec ((__do_loop (lambda (l e d)
                                                                      (if (let ((__or_res (null? e))) (<change> () cdr) (if __or_res __or_res (my-match (cdr pat) d (cons (cons (car pat) l) alist))))
                                                                         (if (null? e) #f #t)
                                                                         (__do_loop
