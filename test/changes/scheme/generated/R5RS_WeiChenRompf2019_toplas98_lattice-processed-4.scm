...
(letrec ((lexico (lambda (base)
         ...
         (lattice-reverse! (letrec ((rotate (lambda (fo fum)
                                             (<change>
                                                ...
                                                ((lambda (x) x) (let ((next (cdr fo))) (set-cdr! fo fum) (if (null? next) fo (rotate next fo))))))))
                             (lambda (lst)
                      ...
                      (<change>
                         ...
                         ((lambda (x) x)
                            (let ((less (select-map (lambda (p) (eq? 'less (scmp (car p) new))) (lambda (l) (cdr l)) pas))
                               ...
                               (zulu-select
                                  (lambda (t)
                                     (if (map-and (lambda (t2) (<change> (memq (tcmp t2 t) (__toplevel_cons 'less (__toplevel_cons 'equal ()))) ((lambda (x) x) (memq (tcmp t2 t) (__toplevel_cons 'less (__toplevel_cons 'equal ())))))) less)
                                        (map-and (lambda (t2) (memq (tcmp t2 t) (__toplevel_cons 'more (__toplevel_cons 'equal ())))) more)
                                        #f))
                                  (lattice->elements target))))))))
